...
- **Bloc NEGATIVE : toujours, partout.** Quelle que soit la langue, quel que soit l'outil. C'est lui qui porte les interdits non négociables (yeux visibles, symbole religieux, allégation santé, matières interdites) — il ne s'abrège pas et ne s'omet pas. *(Hypothèse écartée le 2026-09-11 : « les négatifs seraient contre-productifs sur ChatGPT/Gemini » — c'est vrai des anciens modèles de diffusion, pas des modèles actuels qui suivent correctement les instructions négatives.)*
- **⚠️ La déformation du produit dépend de sa TAILLE dans le cadre** *(règle découverte à la production le 2026-09-11, lot de 6 générations — cf. `ledje-asset-log.md`)*. Bouteille **grande** dans le cadre → forme et étiquette justes. Bouteille **petite ou lointaine** → déformée, étiquette réinventée : sous une certaine taille, le modèle **reconstruit** le produit au lieu de le reproduire. ➡️ **Deux méthodes selon le plan** : **plan serré** (main, macro, packshot) = on génère la bouteille directement ; **plan large** (situation, silhouette, scène habitée) = on génère la scène **sans compter sur la bouteille**, puis on **incruste le détourage**. Ne pas s'acharner à corriger par le prompt : c'est une limite du modèle, pas une erreur de rédaction.
- **📏 Donner la proportion de l'étiquette, chiffrée** *(défaut relevé le 2026-09-11, plan J)*. Livré à lui-même, le modèle rend la bande à ≈ **la moitié** de la hauteur du flacon ; la vraie fait **50 mm sur 146 mm, soit un peu plus du TIERS**. Formulation qui corrige : *« la bande d'étiquette fait environ un tiers de la hauteur de la bouteille, dans sa moitié basse »*. ⚠️ **À ne pas confondre avec la déformation** (règle ci-dessus) : la déformation n'est pas corrigeable par le prompt, **la proportion l'est**.
- **Sur-décrire la condensation.** « Commence à se former » est ignoré. Formulation qui marche : *« de fines gouttes couvrent le plastique, denses vers le haut ; deux ou trois filets ont déjà coulé »*.
